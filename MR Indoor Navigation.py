...
# Graph for dynamic path planning (shortest path logic can be added later)
graph = {
...
path = ["115", "MA155", "MA150", "101"]  # Example path; you can dynamically set it
current_index = 0
...
def start_socket_server():
    ...
    server_socket.bind(('localhost', 65432))  # Same port will be used in Unity
    server_socket.listen(1)
    print("Socket server started, waiting for Unity connection...")
...
full_distance = total_distance(path)
total_time = full_distance / 1.0  # Assume 1 m/s walking speed

print(f"Total distance: {full_distance:.2f} meters")
print(f"Estimated time: {total_time:.1f} seconds")
speak(f"Your path is from {path[0]} to {path[-1]}. Total distance is {full_distance:.0f} meters. Estimated time is {total_time:.0f} seconds.")
...
conn = start_socket_server()  # Unity will connect here
            ...
            if marker_id in marker_id_to_room:
                ...
                if room == path[current_index] and room not in visited:
                    ...
                    if current_index < len(path) - 1:
                        ...
                        data_to_send = f"{room},{next_room},{direction}\n"
                        ...
                        speak(f"Turn {direction} towards {next_room}. Remaining distance is {remaining_distance:.0f} meters. Time left is {remaining_time:.0f} seconds.")
                    else:
                        speak("You have reached your final destination.")
                        data_to_send = f"{room},END,END"
